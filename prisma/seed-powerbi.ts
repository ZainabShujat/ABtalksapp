/**
 * Seed 078 LearningProgram "powerbi" from prisma/content/powerbi-cohort JSON.
...
const PROGRAM_SLUG = "powerbi";
const COHORT_SLUG = "powerbi-open";
...
function assertNotProduction(): void {
  ...
  console.log(`[powerbi-seed] targeting host: ${host}`);
}

function loadJsonFile<T>(filename: string): T {
  const full = path.join(CONTENT_DIR, filename);
  if (!fs.existsSync(full)) {
    throw new Error(`[powerbi-seed] ${filename} not found at ${full}`);
  }
  return JSON.parse(fs.readFileSync(full, "utf-8")) as T;
}

function activityIdForDay(dayNumber: number): string {
  return `act_pbi_day_${String(dayNumber).padStart(2, "0")}`;
}

function activityIdForVideo(dayNumber: number, order: number): string {
  return `act_pbi_vid_${String(dayNumber).padStart(2, "0")}_${String(order).padStart(2, "0")}`;
}

/**
 * `youtubeId` ships as null in powerbi-cohort/videos.json — the ids are curated
 * per cohort run. Nulls are written through to the nullable videoRef column and
 * the day-shell reader skips those rows, so nothing renders until they are set.
 */
type VideoJson = {
  dayNumber: number;
  videos: {
  ...
  try {
    ...
    const category = await prisma.programCategory.upsert({
      ...
      create: {
        ...
        description: "7-Day Power BI & Analytics Cohort.",
        colorToken: "powerbi",
        sortOrder: 40,
        isActive: true,
      },
    ...
    const program = await prisma.learningProgram.upsert({
      ...
      create: {
        slug: PROGRAM_SLUG,
        title: "7-Day Power BI & Analytics Cohort",
    ...
    for (const d of days) {
      ...
      if (!moduleId) {
        throw new Error(
          `[powerbi-seed] day ${d.dayNumber}: module ${d.moduleNumber} not found`,
        );
      }
    ...
    for (const entry of videoDays) {
      ...
      if (!dayMeta) {
        throw new Error(
          `[powerbi-seed] videos: day ${entry.dayNumber} not in days.json`,
        );
      }
      const moduleId = moduleIdByNumber.get(dayMeta.moduleNumber);
      if (!moduleId) {
        throw new Error(
          `[powerbi-seed] videos: module ${dayMeta.moduleNumber} not found`,
        );
      }
    ...
    console.log(
      `[powerbi-seed] upserted program=${PROGRAM_SLUG} modules=${modules.length} days=${days.length} videos=${videoCount} cohort=${COHORT_SLUG}`,
    );
    if (pendingVideoIds > 0) {
      console.warn(
        `[powerbi-seed] ${pendingVideoIds} video(s) have no youtubeId yet — they stay hidden on the day page until curated.`,
      );
    }
  } finally {
