...
export type PowerBiDayState = "LOCKED" | "AVAILABLE" | "PASSED";

/** IST calendar key of the learner's Day 1. */
export function anchorKey(startedAt: Date): string {
  return formatInTimeZone(startedAt, POWERBI_TZ, "yyyy-MM-dd");
}
...
/** Uncapped elapsed day (1-based). Day 8+ is real and means "past the plan". */
export function elapsedDay(startedAt: Date, now = new Date()): number {
  const startKey = anchorKey(startedAt);
  const nowKey = formatInTimeZone(now, POWERBI_TZ, "yyyy-MM-dd");
...
/** Display/unlock ceiling: min(7, elapsedDay). */
export function maxUnlockedDay(startedAt: Date, now = new Date()): number {
  return Math.min(POWERBI_TOTAL_DAYS, elapsedDay(startedAt, now));
}
...
export function latenessForPass(
  ...
  const passKey = formatInTimeZone(at, POWERBI_TZ, "yyyy-MM-dd");
  return passKey <= unlockKey
    ? AttemptLateness.ON_TIME
    : AttemptLateness.LATE;
}

/** max(0, min(7, elapsedDay) - highestPassedDay). Marked, never blocking. */
...
export function todayKey(now = new Date()): string {
  return formatInTimeZone(now, POWERBI_TZ, "yyyy-MM-dd");
}
