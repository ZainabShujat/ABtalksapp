...
export function anchorKey(startedAt: Date): string {
  return formatInTimeZone(startedAt, DATABRICKS_TZ, "yyyy-MM-dd");
}
...
export function elapsedDay(startedAt: Date, now = new Date()): number {
  ...
  const nowKey = formatInTimeZone(now, DATABRICKS_TZ, "yyyy-MM-dd");
...
export function maxUnlockedDay(startedAt: Date, now = new Date()): number {
  return Math.min(DATABRICKS_TOTAL_DAYS, elapsedDay(startedAt, now));
}
...
export function latenessForPass(
  ...
  const passKey = formatInTimeZone(at, DATABRICKS_TZ, "yyyy-MM-dd");
...
export function todayKey(now = new Date()): string {
  return formatInTimeZone(now, DATABRICKS_TZ, "yyyy-MM-dd");
}
